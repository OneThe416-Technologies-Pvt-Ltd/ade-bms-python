...
rs232_device_2_data ={
            ...
            'cell_7_temp': 1,
            'ic_temp': 25,
            'bus_1_voltage_before_diode': 50,
            'bus_2_voltage_before_diode': 50,
            'bus_1_voltage_after_diode': 50,
            'bus_2_voltage_after_diode': 50,
            'bus_1_current_sensor1': 50,
            'bus_2_current_sensor1': 50,
            'charger_input_current': 50,
            'charger_output_current': 50,
            'charger_output_voltage': 50,
            'charging_on_off_status': 1,
            'charger_relay_status': 1,
            'constant_voltage_mode': 1,
            'constant_current_mode': 1,
            'input_under_voltage': 50,
            'output_over_current': 50,
            'bus1_status': 1,
            'bus2_status': 1,
            'heater_pad': 1,
}
...
def update_rs232_device_1_data(data_hex):
    ...
    rs232_device_1_data['input_under_voltage'] = (charger_status_byte & 0x10) >> 4

    # Byte 41: Output Over Current (1 byte)
    rs232_device_1_data['output_over_current'] = (charger_status_byte & 0x20) >> 5

    info_status_byte = data_hex[35]  # Byte 35 in the data

    # Byte 42: Bus1 Status (1 byte)
    rs232_device_1_data['bus1_status'] = (info_status_byte & 0x01)

    # Byte 43: Bus2 Status (1 byte)
    rs232_device_1_data['bus2_status'] = (info_status_byte & 0x02) >> 1

    # Byte 44: Heater Pad Status (1 byte)
    rs232_device_1_data['heater_pad'] = (info_status_byte & 0x04) >> 2
          
    print(f"Updated RS232 device data: {rs232_device_1_data}")
...
def update_rs422_device_1_data(data_bytes):
    ...
    rs422_device_1_data['channel_selected'] = channel_status
    ...
    rs422_device_1_data['state_oc_charge'] = raw_state_of_charge
